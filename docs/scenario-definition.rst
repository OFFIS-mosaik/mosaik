...
time at which is was generated by the source simulator, not with the time at which
the target simulator happens to run).

Usually, the solution to resolve warnings due to discouraged connections is to
change the affected target simulator from time-based to hybrid and to mark the
corresponding input attribute as trigger in the simulator's ``META``. Occasionally,
you might want to change the source simulator to hybrid, instead. (And then change
