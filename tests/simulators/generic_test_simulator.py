...
step_type : string, {'time-based', 'event-based', 'hybrid'}, default
    'time-based')
step_size : int, default=1, only used by time-based simulators
self_steps (dict of {int: int}, default {}), only used by event-based
    and hybrid simulators {step_time: next_step, ..}
    A next step will only be requested (at next_step) for time steps
    which are in self_steps.
wallclock_duration : float, default 0.
    If set, the simulator will sleep for this time (in seconds) during
    step.
output_timing : dict of {int: int}, optional
    ...
    output_timing.
    Can also contain a list of output_times: {int: [int, int, ..]} for
    same time loops with multiple steps within the same step_time.
events : dict of {float: int}, default {}
    {real_time: event_time, ..}
    An event will be requested for simulation time event_time after
    real_time seconds.
"""
...
class TestSim(mosaik_api_v3.Simulator):
    ...
    def init(
        ...
        if output_timing:
            output_timing = {float(key): val for key, val in output_timing.items()}
