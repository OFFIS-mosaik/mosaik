...
def test_transform_parameter_input_pushed():
    ...
    world.connect(
        input_model_const[0], output_model[0], "static_value", transform=multiply_by_ten
    )

    world.connect(
        input_model_const[0], output_model[1], "value", transform=multiply_by_onehundred
    )
    ...
    for key in test_dict:
        ...
        assert test_dict[key]["static_value"][input_model_const[0].full_id] == 10
    for key in test_dict2:
        assert test_dict2[key]["value"][input_model_const[0].full_id] == 200
